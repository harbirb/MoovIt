...
const ActivitySoundtrack = mongoose.model(
  "ActivitySoundtrack",
  activitySoundtrackSchema
);
...
app.get("/api/first50/:time", async (req, res) => {
  ...
  const songsBeforeEndResponse = await fetch(
    `https://api.spotify.com/v1/me/player/recently-played?limit=50&after=${end_time}`,
    {
      method: "GET",
      headers: {
...
app.get("/api/testpage/:activity", async (req, res) => {
  const soundtrack = await getActivitySoundtrack(
    req.session.athlete_id,
    req.params.activity
  );
  res.send(soundtrack);
});
...
app.get("/auth/strava/callback", async (req, res) => {
  ...
  try {
    const { athlete_id, ...tokenData } = await exchangeStravaAuthCodeForTokens(
      AUTH_CODE
    );
...
async function updateUserStravaTokens(
  athlete_id,
  access_token,
  refresh_token,
  expires_at
...
async function getStravaToken(athlete_id) {
  ...
  try {
    const { access_token, refresh_token, expires_at } =
      await refreshStravaTokens(user.stravaRefreshToken);
...
async function getSpotifyToken(athlete_id) {
  ...
  try {
    const { access_token, expires_at } = await refreshSpotifyTokens(
...
async function getActivitySoundtrack(athlete_id, activity_id) {
  ...
  const activitySoundtrackDocument = await ActivitySoundtrack.findOne({
  ...
  try {
    const soundtrack = await generateActivitySoundtrack(
      athlete_id,
      activity_id
    );
    ...
    console.error(
      `Error in fetching soundtrack for activity ${activity_id}`,
      error
    );
  }
}
...
async function generateActivitySoundtrack(athlete_id, activity_id) {
  ...
  try {
    ...
    const songsDuringActivity = songsAfterStart.items.filter((obj) =>
      songSet.has(obj.played_at)
    );
